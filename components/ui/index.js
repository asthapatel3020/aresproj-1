...
UI.Pager = ({currentPage, itemsPerPage, pages, onNext, onPrevious, onPage, classes}) => {
  let buttonArray = [];
  ...
  const linkButtons = buttonArray.map(index => {
    return <li key={'page-key-' + index}   className="paginate_button " aria-controls="DataTables_Table_1" tabIndex="0"><a href="javascript:;" style={{background:currentPage==(index+1)&&'rgb(255, 195, 51)',color:currentPage==(index+1)&&'#fff',fontWeight:'900' }} onClick={()=>onPage(index+1)}>{index+1}</a></li>
  })
      ...
      <div>
          <div className="dataTables_paginate paging_simple_numbers text-right" id="DataTables_Table_1_paginate">
              <ul className="pagination" style={{marginTop:5}}>
                  {currentPage!==1&&<li className="paginate_button previous " aria-controls="DataTables_Table_1" tabIndex="0" id="DataTables_Table_1_previous"><a href="javascript:;" onClick={onPrevious}>{"<"}</a></li>}
                  {linkButtons}
                  {currentPage!==pages&&<li className="paginate_button next" aria-controls="DataTables_Table_1" tabIndex="0" id="DataTables_Table_1_next"><a href="javascript:;" onClick={onNext}>{">"}</a></li>}
              </ul>
          </div>
      </div>
    </div>
