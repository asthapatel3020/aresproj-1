...
class ArbitrationByProvider extends Component  {
	state={
		tab:0
	}
	...
	closeModal() {
		this.props.onClose(5)
		this.props.dispatch(reset('arbitrationByProviderForm'))

	}
	changeTabToReport() {
		this.setState({tab:1})
		this.props.dispatch(submit('arbitrationByProviderForm', ''))
	}
	render() {
		...
		return (
			<Modal 
				...
				<div style={{ width:'100%', background:'#fff', height:'100%', paddingBottom:'2rem', overflow:'auto'}}>
					<div className="modal-title">Arbitration by lawyer</div>
...
export default connect(mapStateToProps)(ArbitrationByProvider);
