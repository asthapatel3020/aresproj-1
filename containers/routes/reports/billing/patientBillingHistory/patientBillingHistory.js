...
class PatientBillingHistory extends Component  {
	state={
		tab:0
	}
	...
	closeModal() {
		...
		this.props.dispatch(reset('patientBillingHistory'))

	}
	changeTabToReport() {
		this.setState({tab:1})
		// this.props.dispatch(submit('paymentsByProviderForm', ''))
	}
	render() {
		...
		return (
			<Modal 
				...
				<div style={{ width:'100%', background:'#fff', height:'100%', paddingBottom:'2rem', overflow:'auto'}}>
					<div className="modal-title">Patient billing history</div>
					<div style={{padding:'1rem'}}>
						<div className="d-flex">
	              			<div style={{marginRight:15}}>
	              				<Button  variant={'fab'} type='print' color='secondary' />
	              			</div>
						...
						<div style={{padding:'20px 0'}}>
							<div style={{width:'100%'}} className="d-flex flex-column align-items-start">
								...
								{tab==0&&<Form changeTabToReport={()=>this.changeTabToReport()}/>
								}
								{tab==1&&
									<Report />}
...
export default connect(mapStateToProps)(PatientBillingHistory);
