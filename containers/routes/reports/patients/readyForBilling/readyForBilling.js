...
class ReadyForBilling extends Component  {
	state={
		tab:0
	}
	...
	closeModal() {
		this.props.onClose(8)
		this.props.dispatch(reset('readyForBillingForm'))

	}
	changeTabToReport() {
		this.setState({tab:1})
		this.props.dispatch(submit('readyForBillingForm', ''))
	}
	render() {
		...
		return (
			<Modal 
				...
				<div style={{ width:'100%', background:'#fff', height:'100%', paddingBottom:'2rem', overflow:'auto'}}>
					<div className="modal-title">Ready for billing</div>
...
export default connect(mapStateToProps)(ReadyForBilling);
