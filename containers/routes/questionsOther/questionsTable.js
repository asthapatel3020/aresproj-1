...
class QuestionsTable extends Component  {
	...
	handleChangeClick(e,item) {
		this.props.dispatch(actions.setEditQuestionOther(item))

		this.props.router.push(`/questionsother/edit`)
	}
	render(){
		console.log("QQQ", this.props.userAccess)
		const {props} = this;
	return<div style={{marginLeft:'-3.4%', marginRight:'-3.4%'}}> 
		<div style={{display:'flex', justifyContent:'space-between', padding:'0 3%'}}>
			            ...
			            <div className="question-btns">
		            		...
		            		{this.props.userAccess==1&&<Button onClick={()=>this.props.router.push('/questionsother/create_question')} label="Добавить" style={{textAlign:'right'}} size="btn-sm" color="btn-warning"   /> }
						</div>
